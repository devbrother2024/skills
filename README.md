...
- `<스킬>`: 위 Skills 표의 이름
- `<에이전트>`: `claude-code` | `codex` | `cursor` 등. [skills.sh가 지원하는 에이전트](https://www.skills.sh/agent)라면 어디에나 설치할 수 있습니다 (GitHub Copilot, Gemini, Windsurf 등 20종)
...
**Claude Code**: 별도 등록 없이 위 설치만으로 동작합니다. 채팅에서 `/`를 입력했을 때 스킬 이름이 자동완성에 뜨면 설치된 것입니다. 스킬은 전역 `~/.claude/skills` 또는 프로젝트 `.claude/skills`에 들어갑니다. plugin marketplace로도 설치할 수 있습니다.

```
/plugin marketplace add devbrother2024/skills
/plugin install devbrother-skills@devbrother-skills
```
