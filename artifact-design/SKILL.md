...
description: >
  랜딩 페이지·문서·대시보드·데모·게임, 그리고 프론트엔드 구현 전 UX/UI 대안·시안을
  나란히 비교하는 목업 같은 self-contained HTML 산출물을 만들 때 팔레트·타이포그래피·
  레이아웃을 의도적으로 설계하고, 템플릿·AI스러운 디자인을 피하도록 돕는 디자인 가이드.
  HTML/CSS를 쓰기 전에 로드해, 작업이 실제로 요구하는 treatment에 맞춰 설계한다.
  "랜딩 만들어줘", "이 문서/페이지 보기 좋게", "대시보드 UI 짜줘", "HTML로 예쁘게 만들어줘",
  "디자인 잡아줘", "이 UX 대안들 비교해줘", "개발 전에 시안 몇 개 뽑아줘", 또는 웹 페이지·
  화면·컴포넌트·시각 산출물을 만들거나 비교하는 요청에 활성화한다. 순수 백엔드·로직·
  CLI 작업이나 텍스트만 필요한 작업에는 사용하지 않는다.
---
