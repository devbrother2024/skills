...
산출물은 **self-contained `.html` 파일** 하나다. CSS·JS는 인라인하고, 에셋은 `data:` URI로 임베드한다. 파일을 대신 렌더링해 주는 호스팅 환경이 없으므로, 브라우저에서 `file://`로 바로 열든 정적 서버에 올리든 그대로 동작해야 한다. 완성하면 가능한 수단으로 한 번 실제 렌더링을 확인하라. 문법 검증만으로는 화면 깨짐을 잡지 못한다.
