...
- **미커밋 변경 없이 미푸시 커밋만 있으면**: push 전에 Step 8의 best-effort 검증(테스트/린트/빌드)을 먼저 수행한다. 통과하거나 감지되는 검증 수단이 없으면 자동 push하고 Step 8의 대기·재조회 루프로 진입한다. **검증이 실패하면 push하지 않고 중단하며**, 이전 실행이 검증 실패로 남겨둔 커밋일 수 있음을 알리고 실패 내용을 사용자에게 보고한다(이미 검사에 실패한 커밋을 재검증 없이 게시하지 않기 위함이다).
